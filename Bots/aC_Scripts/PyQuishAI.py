...
# Move DrawWindow function before main()
def DrawWindow():
    if PyImGui.begin(module_name, PyImGui.WindowFlags.AlwaysAutoResize):

        # Start / Stop button
        if PyImGui.button("Start bot" + " " + IconsFontAwesome5.ICON_PLAY_CIRCLE if not bot_vars.is_running else "Stop bot" + " " + IconsFontAwesome5.ICON_STOP_CIRCLE, width=140):
        ...
        if PyImGui.button("Pause bot" + " " + IconsFontAwesome5.ICON_PAUSE_CIRCLE if not bot_vars.is_paused else "Resume", width=140):
            TogglePause()

        PyImGui.separator()

        # Region Selection
        PyImGui.text(IconsFontAwesome5.ICON_GLOBE_EUROPE + "   " +  "Select Region:")
        ...
        if region_index < len(regions):
            ...
            if bot_vars.selected_region != region:
                bot_vars.selected_region = region
        ...
        if bot_vars.selected_region:
            ...
            PyImGui.text(IconsFontAwesome5.ICON_MAP + "   " +  "Select Map:")
            map_index = PyImGui.combo("##Map", map_index, maps)
            if map_index < len(maps):
                selected_map = maps[map_index]
                if bot_vars.selected_map != selected_map:
                    bot_vars.selected_map = selected_map
                    load_map_script()
        ...
        # Statistics
        PyImGui.text(IconsFontAwesome5.ICON_LIST_ALT + "   " + "Statistics:")
        ...
        PyImGui.separator()
        PyImGui.text(IconsFontAwesome5.ICON_TROPHY + "   " + "Relevant - Title / Alligiance:")
        PyImGui.separator()
        ...
        if region in kurzick_regions:
            ...
            for title_id, title_name, tier_data in eotn_region_titles[region]:
                display_title_track(title_name, title_id, tier_data)
        # Player Position
        #player_x, player_y = Agent.GetXY(Player.GetAgentID())
        #PyImGui.text(f"Position: ({player_x:.0f}, {player_y:.0f})")
