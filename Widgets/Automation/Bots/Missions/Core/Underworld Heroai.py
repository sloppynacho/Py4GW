...
_HEROAI_MISSING_FEATURE_LOGGED: set[str] = set()


def _log_missing_heroai_feature(function_name: str) -> None:
    if function_name in _HEROAI_MISSING_FEATURE_LOGGED:
        return
    _HEROAI_MISSING_FEATURE_LOGGED.add(function_name)
    ConsoleLog(
        BOT_NAME,
        f"[HeroAI] Missing function: {function_name} (no direct HeroAI equivalent).",
        Py4GW.Console.MessageType.Warning,
    )


def _iter_heroai_emails() -> list[str]:
    emails: list[str] = []
    ...
    if not emails:
        fallback_email = Player.GetAccountEmail()
        if fallback_email:
...
def _set_heroai_option_for_party(option_name: str, enabled: bool) -> None:
    shmem = GLOBAL_CACHE.ShMem
    updated = 0
    for email in _iter_heroai_emails():
        options = shmem.GetHeroAIOptionsFromEmail(email)
        if options is None or not hasattr(options, option_name):
            continue
        setattr(options, option_name, bool(enabled))
        shmem.SetHeroAIOptionsByEmail(email, options)
        updated += 1

    if updated == 0:
        ConsoleLog(
            BOT_NAME,
            f"[HeroAI] Could not update option '{option_name}' for any account.",
            Py4GW.Console.MessageType.Warning,
        )


def _set_party_following_enabled(enabled: bool) -> None:
    _set_heroai_option_for_party("Following", enabled)


def _set_party_looting_enabled(enabled: bool) -> None:
    _set_heroai_option_for_party("Looting", enabled)


def _set_party_targeting_enabled(enabled: bool) -> None:
    _set_heroai_option_for_party("Targeting", enabled)


def _set_party_custom_target(agent_id: int) -> None:
    # Missing HeroAI function: set_party_custom_target(target_id)
    # Fallback: local target change + enable HeroAI targeting.
    _log_missing_heroai_feature("set_party_custom_target")
    if Agent.IsValid(agent_id):
        Player.ChangeTarget(agent_id)
        _set_party_targeting_enabled(True)


def _set_party_forced_state(state_name: str | None) -> None:
    # Missing HeroAI function: set_party_forced_state(state)
    _log_missing_heroai_feature("set_party_forced_state")


def _set_party_leader_email(_email: str) -> None:
    # Missing HeroAI function: set_party_leader_email(email)
    _log_missing_heroai_feature("set_party_leader_email")


def _set_party_blessing_enabled(_enabled: bool) -> None:
    # Missing HeroAI function: set_party_is_blessing_enabled(enabled)
    _log_missing_heroai_feature("set_party_is_blessing_enabled")


def _auto_assign_flag_emails() -> None:
    # Missing HeroAI function: auto_assign_emails_if_none_assigned()
    _log_missing_heroai_feature("auto_assign_emails_if_none_assigned")
...
def _set_flag_position(index: int, flag_x: int, flag_y: int) -> None:
    hero_party_position = index + 1
    hero_agent_id = GLOBAL_CACHE.Party.Heroes.GetHeroAgentIDByPartyPosition(hero_party_position)
    if hero_agent_id:
        GLOBAL_CACHE.Party.Heroes.FlagHero(hero_agent_id, flag_x, flag_y)


def _toggle_wait_if_aggro(enabled: bool) -> None:
    # Missing HeroAI function: wait_if_in_aggro
    _log_missing_heroai_feature("wait_if_in_aggro")


def _toggle_wait_for_party(enabled: bool) -> None:
    # Missing HeroAI function: wait_if_party_member_too_far
    _log_missing_heroai_feature("wait_if_party_member_too_far")


def _toggle_move_if_aggro(enabled: bool) -> None:
    # Missing HeroAI function: move_to_party_member_if_in_aggro
    _log_missing_heroai_feature("move_to_party_member_if_in_aggro")


def _toggle_move_to_enemy_if_close_enough(enabled: bool) -> None:
    _set_party_targeting_enabled(enabled)


def _toggle_move_to_party_member_if_dead(enabled: bool) -> None:
    # Missing HeroAI function: move_to_party_member_if_dead
    _log_missing_heroai_feature("move_to_party_member_if_dead")


def _toggle_wait_if_party_member_needs_to_loot(enabled: bool) -> None:
    # Missing HeroAI function: wait_if_party_member_needs_to_loot
    _log_missing_heroai_feature("wait_if_party_member_needs_to_loot")


def _toggle_lock(enabled: bool) -> None:
    # Missing HeroAI function: wait_if_lock_taken
    _log_missing_heroai_feature("wait_if_lock_taken")


def _toggle_wait_if_party_member_mana_too_low(enabled: bool) -> None:
    # Missing HeroAI function: wait_if_party_member_mana_too_low
    _log_missing_heroai_feature("wait_if_party_member_mana_too_low")


def _setup_custom_behavior_integration(bot_instance: Botting) -> None:
    bot_instance.Templates.Multibox_Aggressive()


def _sync_custom_behavior_runtime() -> None:
    # No custom behavior runtime to sync; HeroAI options are updated directly.
    return
...
def _reactivate_custom_behavior_for_step(bot_instance: Botting, step_label: str) -> None:
    _setup_custom_behavior_integration(bot_instance)
...
def _flag_both(party_pos: int, flag_index: int, x, y) -> None:
    # HeroAI/native: setzt Flagge per Party API
...
def _enqueue_spread_flags(bot_instance: Botting, flag_points: list[tuple[int, int]]) -> None:
    """Clear flags, then set HeroAI/native flags for each position.
    Only heroes are flagged (player/party leader is excluded automatically)."""
    bot_instance.States.AddCustomState(
        lambda: _clear_all_flags(),
        "Clear Flags",
    )
    bot_instance.States.AddCustomState(
    ...
    for idx, (flag_x, flag_y) in enumerate(flag_points):
        bot_instance.States.AddCustomState(
            ...
            f"Set HeroAI Flag {idx}",
        )
        bot_instance.Party.FlagHero(idx + 1, flag_x, flag_y)
...
def FocusKeeperOfSouls(bot_instance: Botting):
    ...
    def _focus_logic():
        ...
        _set_party_custom_target(closest_enemy)
...
def bot_routine(bot: Botting):
    ...
    _set_party_blessing_enabled(True)
...
def Enter_UW(bot_instance: Botting):
    ...
    _set_party_leader_email(Player.GetAccountEmail())
    bot_instance.Items.UseItem(3746)  # Passage Scroll to the Underworld
...
def enable_default_party_behavior(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(lambda: _set_party_following_enabled(True), "Enable Follow")
    bot_instance.States.AddCustomState(lambda: _set_party_looting_enabled(True), "Enable Looting")
...
def Clear_the_Chamber(bot_instance: Botting):
    ...
    bot_instance.States.AddCustomState(lambda: _set_party_forced_state("CLOSE_TO_AGGRO"),"Force Close_to_Aggro",)
    ...
    bot_instance.States.AddCustomState(lambda: _set_party_forced_state(None),"Release Close_to_Aggro",)
...
def Wrathfull_Spirits(bot_instance: Botting):
    ...
    if BotSettings.WrathfullSpirits:
        ...
        # Missing HeroAI function: set_party_is_combat_enabled(False)
        bot_instance.States.AddCustomState(
        ...
        # Missing HeroAI function: set_party_is_combat_enabled(True)
        bot_instance.States.AddCustomState(lambda: _toggle_wait_for_party(True), "Enable WaitIfPartyMemberTooFar") 
        bot_instance.States.AddCustomState(lambda: _toggle_wait_if_aggro(True), "Enable WaitIfInAggro")
...
def Unwanted_Guests(bot_instance: Botting):
    ...
    if BotSettings.UnwantedGuests:
        ...
        bot_instance.Wait.ForTime(5000)
        bot_instance.States.AddCustomState(lambda: _set_party_following_enabled(False), "Disable Following")
        bot_instance.States.AddCustomState(lambda: _set_party_forced_state("CLOSE_TO_AGGRO"),"Force Close_to_Aggro")
        ...
        bot_instance.States.AddCustomState(lambda: _set_party_following_enabled(True), "Enable Following")
        bot_instance.States.AddCustomState(lambda: _set_party_forced_state(None),"Release Close_to_Aggro",)
        bot_instance.States.AddCustomState(lambda: _toggle_wait_for_party(True), "Enable WaitIfPartyMemberTooFar")
...
def The_Four_Horsemen(bot_instance: Botting):
    ...
    if BotSettings.TheFourHorsemen:
        ...
        bot_instance.States.AddCustomState(lambda: _set_party_looting_enabled(False), "Disable Looting")
        bot_instance.States.AddCustomState(lambda: _toggle_wait_for_party(False), "Disable WaitIfPartyMemberTooFar")
...
def _draw_help():
    ...
    PyImGui.text("This variant uses HeroAI/native controls; missing HeroAI features are marked in code comments.")
