...
def RegisterCandidate(cache_data):
    ...
    try:
        if not cache_data.is_outpost:
            return False
        
        if cache_data.party_leader_id == cache_data.player_agent_id:
            cache_data.HeroAI_vars.submit_candidate_struct.PlayerID = cache_data.player_agent_id
            cache_data.HeroAI_vars.submit_candidate_struct.MapID = cache_data.map_id
            cache_data.HeroAI_vars.submit_candidate_struct.MapRegion = cache_data.region
            cache_data.HeroAI_vars.submit_candidate_struct.MapDistrict = cache_data.district
...
def SendPartyCommand(index, cache_data, command="Invite"):
    try:
        ...
        cache_data.HeroAI_vars.all_candidate_struct[index].InvitedBy=cache_data.player_agent_id
        updated_candidate = CandidateStruct(
                    PlayerID=candidate.PlayerID,
                    MapID=candidate.MapID,
                    MapRegion=candidate.MapRegion,
                    MapDistrict=candidate.MapDistrict,
                    SummonedBy=cache_data.player_agent_id if command == "Summon" else 0,
                    InvitedBy=cache_data.player_agent_id  if command == "Invite" else 0,
                )
...
def ProcessCandidateCommands(cache_data):
    global MAX_NUM_PLAYERS
    try:
        if not cache_data.is_outpost or not cache_data.is_map_ready:
            return False
        ...
        for index in range(MAX_NUM_PLAYERS):
            candidate = cache_data.HeroAI_vars.all_candidate_struct[index]
            if candidate.PlayerID == cache_data.player_agent_id:
                self_index = index
        ...
        for index in range(MAX_NUM_PLAYERS):
            ...
            if (candidate.PlayerID == cache_data.player_agent_id) and (candidate.InvitedBy != 0):      
                updated_candidate = CandidateStruct(
                    PlayerID=candidate.PlayerID,
                    MapID=candidate.MapID,
                    MapRegion=candidate.MapRegion,
            ...
            if (candidate.PlayerID == cache_data.player_agent_id) and (candidate.SummonedBy != 0):
