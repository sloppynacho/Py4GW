...
class WidgetHandler:
    ...
    def _read_enabled_state(self, widget_id: str, default: bool = False) -> bool:
        ...
        if IniManager().has_key(self.MANAGER_INI_KEY, canonical_section, "enabled"):
            return IniManager().read_bool(self.MANAGER_INI_KEY, canonical_section, "enabled", default)
        ...
        if not IniManager().has_key(self.MANAGER_INI_KEY, legacy_section, "enabled"):
            return bool(default)

        migrated = IniManager().read_bool(self.MANAGER_INI_KEY, legacy_section, "enabled", default)
