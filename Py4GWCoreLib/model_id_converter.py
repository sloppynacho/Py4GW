...
# Input: Your C# list as a multi-line string
csharp_input = """
...
pattern = re.compile(
    r'Npc\s+(?P<key>\w+)\s*=\s*new\(\)\s*\{\s*Ids\s*=\s*new\s*int\[\]\s*\{(?P<ids>[^\}]+)\},\s*Name\s*=\s*"(?P<name>[^"]+)"(?:,\s*WikiUrl\s*=\s*"(?P<wiki_url>[^"]+)")?\s*\};'
)

# Parse the C# input
npcs = []
for match in pattern.finditer(csharp_input):
    ids = list(map(int, match.group('ids').split(',')))  # Convert IDs to a list of integers
    name = match.group('name')
    wiki_url = match.group('wiki_url')  # Optional URL
    npc_entry = {"model_ids": ids, "name": name}
    if wiki_url:
